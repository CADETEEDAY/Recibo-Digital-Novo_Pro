...
class ReciboSoftwareApp(ctk.CTk):
    ...
    def _modal_relatorio(self, rotulo, tipo_filtro):
        ...
        def process():
            ...
            html = f"""
            <!DOCTYPE html><html lang="pt-PT"><head><meta charset="UTF-8"><title>Relatório - {rotulo}</title>
            <style>
        ...
        if not rows:
            with self.db.get_connection() as conn:
                conn.execute("""
                    ...
                    VALUES ('Carlos Eduardo de Souza S.A.', 'Jurídica', '12.345.678/0001-90', 'contato@carlos-desouza.co', '(11) 98765-4321', 'Av. Paulista, 1000 - Cj 42', 'Bela Vista', 'São Paulo', 'SP', '01310-100', 'Cliente preferencial, faturação mensal via PIX.', 'ATIVO')
                """)
                conn.execute("""
                    INSERT INTO clientes (nome, tipo_pessoa, cpf, email, tel1, endereco, bairro, cidade, estado, cep, obs, status)
                    VALUES ('Maria Clara Oliveira', 'Física', '12.345.600-90', 'maria.clara@gmail.com', '(21) 99887-6655', 'Rua das Laranjeiras, 45', 'Laranjeiras', 'Rio de Janeiro', 'RJ', '22240-000', 'Pagamento pronto.', 'ATIVO')
                """)
                conn.commit()
            with self.db.get_connection() as conn:
                rows = conn.execute("SELECT * FROM clientes ORDER BY id DESC LIMIT 10").fetchall()
    ...
    def _salvar_cliente_form(self):
        ...
        if not nome:
            messagebox.showwarning("Atenção", "O Nome ou Razão Social é obrigatório.")
            return
        ...
        messagebox.showinfo("Sucesso", "Registo guardado com sucesso.")
        self.navegar("cadastros")
    ...
    def _excluir_cliente_db(self):
        if not self.cliente_id_atual:
            ...
            return
        if messagebox.askyesno("Confirmar", "Deseja remover o registo selecionado?"):
            with self.db.get_connection() as conn:
    ...
    def _executar_backup(self):
        dest = filedialog.asksaveasfilename(defaultextension=".db", filetypes=[("Base SQLite", "*.db")], initialfile=f"backup_recibo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db")
        if dest:
            try:
                shutil.copy(self.db.db_path, dest)
                messagebox.showinfo("Cópia de Segurança", "Cópia local exportada com sucesso.")
            except Exception as e:
                messagebox.showerror("Erro", f"Falha na cópia: {e}")

    # ==============================================================================
    # MOTOR DE IMPRESSÃO DE RECIBOS
    # ==============================================================================
    def imprimir_recibo(self, vias=1):
        ...
        if not num or not cli or not val:
            messagebox.showwarning("Atenção", "Preencha número, cliente e valor para emitir o recibo.")
            return

        try:
            val_f = float(val)
        except ValueError:
            messagebox.showerror("Erro", "Valor numérico introduzido é inválido.")
            return
        ...
        html = f"""
        <!DOCTYPE html><html lang="pt-PT"><head><meta charset="UTF-8"><title>Recibo Nº {num}</title>
        <style>
        ...
        <body>
            <div class="container-folha">
                <button class="btn-print" onclick="window.print()" style="padding: 10px 20px; margin-bottom: 15px; background: #3b82f6; color: #fff; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 14px;">Imprimir / Guardar em PDF</button>
                {bloco_recibo}
                {segunda_via}
            </div>
        </body></html>
